...
export default function Register() {
    ...
    useEffect(() => {
        if (registerResult.data) {
            if (registerResult.data.register) {
                document.cookie = 'token=' + registerResult?.data?.user?.token;
            }

            notification({
                type: registerResult.data.register ? 'success' : 'error',
                messages: registerResult.data.register ? 'Registered' : registerResult.data.error,
            });
        }
    ...
    if (registerResult.isLoading) {
        ...
        content = <div>Error at Register</div>;
    } else if (user?.id || registerResult.data?.register) {
        content = <Navigate to="/" replace />;
    } else {
        content = (
            <div className="w-full h-screen  flex justify-center items-center p-2">
                <div
                    className="w-full max-w-lg bg-white px-3 py-4 rounded-md h-full max-h-[35.5rem] overflow-auto 
            shadow-[0px_0px_25px_18px_rgba(0,0,0,0.75)]"
                >
                    <div>
                        <h1 className="text-2xl font-semibold max-[300px]:text-xl">Sign up for NewsFeed</h1>
                        <div className="font-medium text-sm max-[300px]:text-xs">
                            Create a free account or{' '}
                            <Link to="/login" className="text-[#7CACAB]">
                                to login
                            </Link>
                        </div>
                    </div>
                    <div>
                        <Formik
                            ...
                            onSubmit={(values) => {
                                ...
                                register({ ...values, password: SHA256(password).toString() });
                            }}
                        >
